...
SELECT
    EXTRACT(QUARTER FROM submission_time::timestamp) AS calendar_quarter,
    COUNT(*)                                         AS total_reviews,
    ROUND(AVG(rating)::numeric, 2)                   AS avg_rating,
    ROUND(SUM(revenue_proxy)::numeric, 2)            AS total_revenue_proxy,
    ROUND(
        (SUM(revenue_proxy) / SUM(SUM(revenue_proxy)) OVER () * 100)::numeric, 2
    ) AS pct_of_total_revenue
FROM reviews
GROUP BY EXTRACT(QUARTER FROM submission_time::timestamp)
